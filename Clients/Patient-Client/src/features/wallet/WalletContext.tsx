...
import { clearPassport, loadPassport } from "@/features/passport/passport-store";
...
/**
 * Owns the connected account and, with it, the passport read.
 *
 * An account appearing is the event that makes passport data fetchable, so the
...
export function WalletProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    walletAdapter
      ...
      .then((restored) => {
        if (cancelled) return;
        if (restored) void loadPassport(restored);
        setAddress(restored);
      })
      .catch(() => {
